...
客户端访问后台，首先经过服务器的路由器，Nginx负责负载均衡，可以负载5万的并发。Keepalived是虚拟路由器，VIP是虚拟IP，当Nginx挂掉时，可以将VIP发送到Keepalived中，防止Nginx的单点故障问题。

Nginx还可以进行限流，通过并发次数，访问IP进行限流，还自带缓存，可减轻访问对后台的压力。后期还会添加OpenRestry，增强并发能力。
...
修改了IP为192.168.112.138

将mysql的权限设置为了root用户能够通过远程访问
...
数据库的设计：微服务的各个表之间的关联关系很弱，在大型项目中，不推荐使用三范式，而是使用反三范式，建议表中不要创建外键关联，在表中搜索时就不需要检查依赖问题，提高相应速度，转而在表中添加冗余数据，将之前需要用外键关联的数据添加到表中，不需要外键操作。

结构说明：

changgou_auth

Oauth2.0授权管理认证中心，即用户的登录需要经过他才能够访问之后的微服务。
...
```
网关模块，根据网站的规模和需要，可以将综合逻辑相关的服务用网关路由组合到一起。在这里还可以做鉴权和限流相关操作。一般项目需要多个网关，如本项目有后台管理员和用户中心，因此每个都需要单独的微服务网关，并且每个网关都需要使用集群来抗住并发的访问。
```
...
```
上面微服务工程中的每个微服务中都有独立的JavaBean，而不同微服务之间需要互相调用，api工程抽取出各个微服务的JavaBean、Feign、以及Hystrix配置，该工程主要对外提供依赖。
```
...
创建父工程 changgou-parent  ,pom.xml文件中增加配置，该父工程为所有项目的父工程，因此直接导入spring-boot-starter-parent，同时导入测试包，fastjson,swagger-ui。父工程都要达成pom包，其他的工程打成jar包。
...
创建changgou-gateway、changgou-service、changgou-service-api、changgou-web工程，父工程全部为pom工程，并将所有工程的src文件删除。
...
创建模块changgou-eureka ，pom.xml引入依赖 ，该模块不是父工程，因此不用打包，默认是JAR包。
...
创建changgou-service-api子模块changgou-service-goods-api，并将`资料\javabean\changgou-service-goods-api`中的Pojo导入到工程中。 **POJO要导入api工程中**

![1560416099767](image\1560416099767.png)



导入POJO后，发现有注解报错。因为有些注解需要导入JPA的依赖包，因此我们可以在api的父工程中导入JPA的依赖包。

修改父工程changgou-service-api的pom.xml，添加`persistence-api`和`changgou-common`的依赖，代码如下：如果导入依赖后依然报错，在导入依赖没问题的前提下，使用maven的compile命令进行编译，IDEA依赖报错的原因是因为没有找到编译文件。



POJO中的注解有JPA的注解，也有Swagger文档的注解，用于抽取到API文档中。
...
在changgou-service中创建changgou-service-goods ，pom.xml引入API的POJO依赖。
...
由于我们使用的是通用Mapper，不是原生的Mybatis，可以不用配置Mybatis。使用通用Mapper只需要继承Mapper类，传入Brand的POJO类的泛型即可，然后调用Mapper类中的固有方法。
...
```
// 只要带有Selective的方法的动态SQL语句 拼接都会忽略空值，更加合理也更加高效，
// 以免加入不无效的空值
```
...
条件搜索查询使用新的代码，请在代码中查看。通过条件搜索构造器来构造搜索条件
...
PageHelper是静态分页工具类，直接进行分页操作，后续需要查看源码来分析。但是后面的查询要紧跟集合查询，不能只查询单个数据。
...
就是将之前的条件查询和分页查询结合在一起了。注意，这里将条件的Example生成单独抽取为一个方法以供按其他方法的调用。
