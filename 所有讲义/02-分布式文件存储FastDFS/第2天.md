...
冗余备份： Storage中每个组的多个节点都是同步备份的，只要一个节点存储了数据，其他节点也会同步保存。

负载均衡： Tracker负责找到适合的Storage组进行存储，分摊存储压力。

线性扩容： 当Storage中组的数量不够时，会自动增加新的组。



FastDFS 架构包括 Tracker server 和 Storage server。客户端请求 Tracker server 进行文件上传、下载，通过Tracker server 调度最终由 Storage server 完成文件上传和下载。

Tracker 和 Storage都是集群，Tracker作为调度器，Storage作为存储器，Storage中分为多个组group，每个组中有多个存储节点，单个组中的所有与节点的关系不是主从关系，而是平级关系。
...
**文件名**：与**文件上传时不同**。是由存储服务器根据**特定信息生成**，文件名包含：源存储

服务器 IP 地址、文件创建时间戳、文件大小、随机数和文件拓展名等信息。

在访问数据时，还要在文件索引前面添加协议，ip，端口号：http://ip:port
...
```properties
docker run -d --name storage --net=host -e TRACKER_IP=192.168.112.138:22122 -e GROUP_NAME=group1 morunchang/fastdfs sh storage.sh
```

- 使用的网络模式是–net=host, 192.168.112.138是宿主机的IP
- -e 代表环境变量，添加宿主机的IP到环境变量中，注意，添加的是连接tracker的IP和端口号22122，还有添加一个组group1
...
当我们访问Storage中的数据时，首先要访问Nginx，这里有个nginx_fastdfs_module模块来路由用户的请求到不同的组中，Nginx在这里主要提供对FastDFS图片访问的支持。

storage容器中已经集成了Nginx，我们可以进入该容器中，进入storage的容器内部，修改nginx的配置nginx.conf
...
可以看到，所有以M00开头的请求都会路由到ngx_fastdfs_module模块中，即用户请求首先经过Nginx，再发送到ngx_fastdfs_module模块中。



为了防止删除文件后，浏览器依然能够通过缓存访问文件，因此需要设置nginx来禁止缓存：
...
创建文件管理微服务changgou-service-file，该工程主要用于实现文件上传以及文件删除等功能，在changgou_service工程下。
...
注意，这里禁止了DataSource的加载创建，因为导入了changgou_common的公共包，会默认加载DataSource，而我们这里没有配置数据库，应用会启动失败，因此禁止DataSource的加载创建。
...
创建Controller来测试存储效果，返回Storage存储URL，拼接成HTTP格式的链接进行访问。



那么，Storage将文件存储在哪里呢？

进入Storage容器中查看，由于使用的是M00虚拟路径，对应的是store_path_0路径，而在Storage的配置文件/etc/fdfs/storage.conf中store_path_0配置的路径是/data/fast_data，进入该路径下的data文件夹，可以看到有许多文件夹，对应的是后面的二级文件路径00/00，根据计算的散列值散列到不同的文件夹中。我们上传的文件就在二级文件路径里面。
...
通过在main函数中获取 Storage中的文件信息测试函数情况。
...
同样的，通过在main函数中将输入流保存到本地磁盘中测试函数情况。
...
现在有个问题是 Storage中删除了文件，但是浏览器中依然有缓存保存着文件，因此我们需要在Nginx中设置禁止缓存，在最上面的Nginx配置中有介绍。
...
得到Tracker的服务端口号为 8080， 而Nginx的端口号也为8080，而请求经过Nginx后，也会访问Tracker，因此我们可以动态地设置Nginx的访问端口为Tracker的端口。



(9)优化

我们可以发现，上面所有方法中都会涉及到获取TrackerServer或者StorageClient，我们可以把它们单独抽取出水电费去，分别在类中添加如下2个方法：
...
规格参数模板是用于管理规格参数的单元。规格是例如颜色、手机运行内存等信息，参数是例如系统：安卓（Android）后置摄像头像素：2000万及以上  热点：快速充电等信息 。用于后台添加商品时，选择商品的各种规格参数。
...
模板与规格是一对多关系 ，模板与参数是一对多关系。一个模板对应着多个规格，多个参数，对于不同的模板展示不同的规格和参数。选择模板（即分类）后，根据模板ID从规格表中选择对应的规格展示，再从参数表中根据模板ID选择相应的参数展示即可。
...
这里注意，每次执行增加和删除的时候，需要调用模板，修改模板中的对应规格数量，另外大家思考下，如果是修改呢，是否会对模板统计数据造成变更呢？
...
| seq         | 排序     | INT      |          |                        |
| parent_id   | 上级ID   | INT      |          | 指向这个表中的父分类Id |
| template_id | 模板ID   | INT      |          |                        |

商品分类与模板是多对一关系， 一个模板对应多个商品分类，即多个商品分类共用一个模板显示。
