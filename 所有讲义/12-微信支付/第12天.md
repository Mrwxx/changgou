...
按API要**求组装参数**，以**XML方式**发送（POST）**给微信支付接口**（URL）,**微信支付接口也是以XML方式给予响应**。程序**根据返回的结果（其中包括支付URL）生成二维码或判断订单状态**。
...
**HttpClient**是Apache Jakarta Common下的子项目，**用来提供高效的、最新的、功能丰富的支持HTTP协议的客户端编程工具包**，并且它支持HTTP协议最新的版本和建议。HttpClient已经应用在很多的项目中，比如Apache Jakarta上很著名的另外两个开源项目Cactus和HTMLUnit都使用了HttpClient。

HttpClient通俗的讲就是**模拟了浏览器的行为，如果我们需要在后端向某一地址提交数据获取结果，就可以使用HttpClient.**

关于HttpClient（原生）具体的使用不属于我们本章的学习内容，我们这里这里为了**简化HttpClient的使用，提供了工具类HttpClient（对原生HttpClient进行了封装）**
...
我们**通过HttpClient工具类实现对远程支付接口的调用**。
...
新增`com.changgou.service.WeixinPayService`接口，**这个接口的参数应该是一个map，便于与XML格式的转换**。代码如下：
...
创建`com.changgou.service.impl.WeixinPayServiceImpl`类,并**发送Post请求获取预支付信息**，**包含二维码扫码支付地址**。代码如下：
...
我们**通过HttpClient工具类实现对远程支付接口的调用**，**自己来创建查询支付状态接口，主动查询支付结果**。
...
在com.changgou.pay.service.impl.WeixinPayServiceImpl中增加实现方法，**封装需要的参数然后通过HTTPClient发送到微信接口中查询订单的支付状态。**
...
**每次添加订单后，会根据订单检查用户是否是否支付成功**，我们不建议每次都操作数据库，每次操作数据库会增加数据库的负载，我们**可以选择将用户的订单信息存入一份到Redis中，提升读取速度**。
...
每次实现支付之后，**微信支付都会将用户支付结果返回到指定路径**，而**指定路径是指创建二维码的时候填写的`notifyurl`参数**,响应的数据以及相关文档参考一下地址：`https://pay.weixin.qq.com/wiki/doc/api/native.php?chapter=9_7&index=8`
...
修改`changgou-service-pay`微服务的com.changgou.pay.controller.WeixinPayController,**添加回调方法**，**回调方法接受一个HttpServletRequest请求，我们可以从这个请求流中取出请求的参数信息，然后读入到一个OutputStream中，继续转换为byte[]数组，String字符串，最终通过微信的SDK中的xmlToMap转换为Map格式对象，这就是微信服务器返回的支付状态信息。**
...
支付系统是与微信服务器对接的系统，是独立于其他系统的服务，不做相关业务逻辑操作，只做支付处理，**所以回调地址接收微信服务返回的支付状态后，立即将消息发送给RabbitMQ**，**订单系统再监听支付状态数据，根据状态数据做出修改订单状态或者删除订单操作。**
...
**修改支付微服务，集成RabbitMQ，因为需要在支付系统中将支付信息发送给MQ**。 添加如下依赖：
...
这里我们**建议在RabbitMQ后台手动创建队列，并绑定队列。如果使用程序创建队列，可以按照如下方式实现。**
...
**创建队列以及交换机并让队列和交换机绑定**，**使用Springframework中的 Environment，直接从配置文件中读取数据。**

修改com.changgou.WeixinPayApplication,添加如下代码：
...
**修改回调方法，在接到支付信息后，立即将支付信息发送给RabbitMQ**，**注入RabbitTemplate对象，使用它来向指定的队列中发送消息。**
...
处理完支付微服务中的发送消息到MQ的功能，接下来 在**订单微服务中，我们需要监听MQ支付状态消息**，并实现订单数据操作。
...
**在application.yml中配置队列名字，便于之后的监听类注解获取**。代码如下：
...
在订单微服务于中创建com.changgou.order.consumer.OrderPayMessageListener，**这是一个监听类，用@RabbitListener注解来标识，其中的queues参数表示监听哪个队列**。并在该类中consumeMessage方法，用于监听消息，获取队列中的消息，**然后根据返回消息中的支付状态处理订单，订单的状态可能有支付成功，则修改为支付成功，否则，支付失败，删除这个订单**。代码如下：
