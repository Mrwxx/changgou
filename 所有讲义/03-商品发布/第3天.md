...
* 概念 : SPU 是商品信息聚合的最小单位，是一组可复用、易检索的标准化信息的集合，该集合描述了一个产品的特性。即同款商品的公共属性抽取出来形成一个表，该款商品所对应的SPU属性不会改变的。SPU表可以关联到SKU表中，将重复的信息抽取到SPU中。
...
* SKU 即**库存进出计量的单位**， 可以是以件、盒、托盘等为单位。
...
* 在服装、鞋类商品中使用最多最普遍。
  ...
  ==某个库存单位的商品独有属性(某个商品的独有属性)==该款商品中可能会改变的属性组成的SKU表，如红色，64G都是可能会改变的。



### 1.2 表结构分析

tb_spu  表 （SPU表），表示一类商品的公共属性，将一类商品中相同的属性抽取到SPU表中，

商品的分类一般是三级分类，即不同的商品只有三个祖先分类。真正的三级分类信息从tb_category表中获取，parent_id子段为0的为一级分类。
...
规格信息是根据template_id 搜索出来的，之前已经选中了分类的商品category_id, 根据该Id从tb_category_brand表中获取该商品分类对应的品牌brand_id，根据该ID再从tb_brand表中获取对应品牌的信息。由于tb_category表中有template_id，根据该Id从tb_spec表中获取该模板对应的信息显示在前端。
...
一会儿会用到ID生成，我们可以使用IdWorker分布式ID生成器，IDWorker在公共工程changgou_common的entity包中，在启动类GoodsApplication中添加如下代码,用于创建IdWorker，并将IdWorker交给Spring容器，代码如下：
...
在实现**商品增加之前**，需要先选择对应的分类，选择分类的时候，首选选择一级分类，然后根据选中的分类，将选中的分类作为查询的父ID，再查询对应的子分类集合，因此我们可以在后台编写一个方法，根据父类ID查询对应的分类集合即可。查询分类当然是从category表中查询，根据parent_id字段的值查询，如果为0则是一级分类，如果不为1则是父分类。
...
修改`com.changgou.goods.dao.BrandMapper`， 自定义一个方法，添加根据分类ID查询对应的品牌数据，代码如下：
...
public interface BrandMapper extends Mapper<Brand> {
    ...
    // 这个sql语句首先满足 tcb的分类ID与传入的参数ID相同，然后通过tcb的品牌ID找出tb对应的品牌所有信息
...
用户选择分类后，需要根据所选分类对象对应的模板ID查询对应的规格，规格表结构如下：
...
前台填写商品信息后，传递到后台的参数是SPU商品公共信息和SKU商品特性信息，SPU信息只有一种，而SKU商品信息有多种，因为会根据特性生成多组商品，因此传递的是一个SPU和多个SKU，SKU使用List来存储。保存商品数据的时候，需要保存Spu和Sku，一个Spu对应多个Sku，我们可以先构建一个Goods对象，将`Spu`和`List<Sku>`组合到一起,前端将2者数据提交过来，再实现添加操作。
...
修改上面的saveGoods方法，当新增一个类型的商品时，如果该种商品在之前已经添加过了，那么重新添加时先将之前的商品删除，再添加新的商品。
