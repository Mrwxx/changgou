...
首页门户系统需要展示各种各样的广告数据。广告数据库为 changgou_content，其中的tb_content表为广告表，tb_content_category表为广告对应的广告分类，不同的广告分区会有不同的价钱，在数据表中有不同的ID表示不同的广告分类。如图，以jd为例：
...
而这里面，我们都可以使用**LUA脚本嵌入到程序**中执行这些查询相关的业务。
...
- 支持面向过程(procedure-oriented)编程和函数式编程(functional programming)；可以用于查询SQL数据库
...
修改/usr/local/openresty/nginx/conf/nginx.conf,将配置文件使用根设置为root,目的就是将来要使用lua脚本的时候 ，直接可以加载在root下的lua脚本。
...
修改nginx配置文件，修改server虚拟机中的拦截请求的地址，将请求的地址与Luau脚本的地址进行匹配，当拦截到该地址后，转到lua脚本中处理。/usr/local/openresty/nginx/conf/nginx.conf文件： 添加头信息，和 location信息
...
重新启动Nginx，执行/nginx/sbin目录下的nginx文件：

./nginx -s reload

由于Nginx配置的就是80端口，因此可以无需写端口号，访问 192.168.112.138/update_content?id=1，页面返回{flag=true}，然后查看Redis中的数据可以看到键“content_1"的值为mysql数据库中tb_content_category中分类1对应的广告。
...
如上的方式没有问题，但是如果请求都到redis，redis压力也很大，所以我们一般采用**多级缓存**的方式来**减少下游系统的服务压力**。参考基本思路图的实现。
...
三层缓存架构，Nginx缓存，Redis缓存，MYSQL，一层层地减轻下游服务器的压力。

![image-20210204102951725](C:\Users\Mr.wmxx\AppData\Roaming\Typora\typora-user-images\image-20210204102951725.png)
...
burst 译为突发、爆发，表示在**超过设定的处理速率后能额外处理的请求数**,当 rate=10r/s 时，将1s拆成10份，即每100ms可处理1个请求。依然是排队处理，而不是并行处理。
...
平均每秒允许不超过2个请求，突发不超过4个请求，**并且处理突发4个请求的时候，没有延迟**，即爆发的流量请求也能够并行处理，等到完成之后，按照正常的速率处理。
...
利用连接数限制 **某一个用户的ip连接的数量**来控制流量。

注意：并非所有连接都被计算在内 只有当服务器正在处理请求并且**已经读取了整个请求头**时，才会计算有效连接。此处忽略测试。
...
canal可以用来监控数据库数据的变化，从而获得新增数据，或者修改的数据。通常用于监控MYSQL数据库，将数据的变化更新到Redis缓存中或者ElasticSearch中，总之就是用于监控数据库变化的，至于要将变化的数据发送给谁，怎么处理，与canal无关。
...
2. mysql master收到dump请求，开始推送binary log给slave(也就是canal)，即将mysql的修改日志发送给了canal
3. canal解析binary log对象(原始为byte流)，得到修改的数据情况。
...
由于canal要查询mysql数据库的情况，因此需要为canal创建一个用户专门用来查询二进制日志数据，使用root账号创建用户并授予权限，mysql的高级用法，看书看书看书！
...
mq config #mq的配置，指定example，当自己定义的canal微服务监听canal时，需要指定example的名称
...
导入idea 打jar包 (步骤 略) 或 **直接进入`starter-canal`目录** 执行mvn命令
...
在target中找到打包好的jar文件 `starter-canal-0.0.1-SNAPSHOT.jar`
...
-Dmaven.repo.local=D:\JAVA\apache-maven-3.5.2\Repository

```

- 文件路径需自行更换自己jar包(starter-canal-0.0.1-SNAPSHOT.jar)的本地路径地址
- 如果maven的仓库地址是默认(${user.home}/.m2/repository)的需去默认路径拷贝到项目的maven仓库路径
- 此时在maven仓库里面可以看到已经安装好的jar包

### 方法2

#### 1. 下载源码文件

#### 2. 切换目录至`starter-canal`
...
4. ###### 去maven仓库查找下载的文件 拷贝至项目对应maven仓库的地址
...
启动失败，由于需要导入canal的客户端依赖，导致了依赖冲突问题。解决流程如下所示：
...
<!--        Canal客户端--> <dependency>     <groupId>com.alibaba.otter</groupId>     <artifactId>canal.client</artifactId>     <version>1.0.24</version>     <exclusions>         <exclusion>             <groupId>org.springframework</groupId>             <artifactId>spring</artifactId>         </exclusion>     </exclusions> </dependency>
...
如上图，每次执行广告操作的时候，会记录操作日志，然后将操作日志发送给canal，canal将操作记录发送给canal微服务，canal微服务根据修改的分类ID调用content微服务查询分类对应的所有广告，canal微服务再将所有广告存入到Redis缓存。
...
content微服务中的方法通过conten_api中的feign接口传递给canal微服务调用，因此canal微服务需要依赖content_api这个项目，才能过通过@EnableFeignClients注解启调用到content_api接口。
...
Canal微服务的监听思路应该是：监听MYSQL数据库的变化情况，
