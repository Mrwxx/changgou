...
用户搜索的时候，除了使用分类、品牌搜索外，还有可能使用规格搜索，所以我们还需要显示规格数据，规格数据的显示相比上面2种实现略微较难一些，需要对数据进行处理，我们也可以考虑使用分类统计和品牌统计的方式进行分组实现。规格数据的显示是根据搜索的SKU的每个spec来对应显示的，将spec中的每个键名抽取为map的键名，将每个规格参数都抽取到set中，防止参数的重复。这样，spec的每个键名都有一个不重复的set集合。
...
**优化分组查询**：如前端已经选择了某一个品牌，那么就不需要对品牌进行分组查询了，因为分组查询的作用是展示所有品牌以供选择。其他的如分类也是如此。
...
页面每次向后台传入对应的分类和品牌，根据分类和品牌过滤SKU商品。
...
PS说明: 以上,我们建议**使用filter ,它的搜索效率要优于must**.可以参考官方文档说明:
...
规格这一块，需要向后台发送规格名字以及规格值，我们可以**按照一定要求来发送数据，例如规格名字以特殊前缀提交到后台：`spec_网络制式：电信4G、spec_显示屏尺寸：4.0-4.9英寸`**

后台接到数据后，可以**根据前缀spec_来区分是否是规格，如果以`spec_xxx`开始的数据则为规格数据**，需要根据指定规格找信息。
...
分页使用nativeSearchQueryBuilder，使用PageRequest.of( pageNo- 1, pageSize);实现，第1个参数表示第N页，从0开始，第2个参数表示每页显示多少条，实现代码如下：
...
排序这里总共有**根据价格排序、根据评价排序、根据新品排序、根据销量排序**，排序要想实现非常简单，只需要告知排序的域以及排序方式即可实现。
...
销量排序：销量排序除了销售数量外，还应该要有时间段限制。发布时间越晚的商品的排名越应该靠前。



### 5.2 排序实现

这里我们不单独针对某个功能实现排序，我们只需要在**后台接收2个参数，分别是排序域名字和排序方式**，代码如下：
