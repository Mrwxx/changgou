...
![1565298291291](images\1565298291291.png)
...
![1565298329794](images\1565298329794.png)
...
**用户的收件地址在用户数据库的tb_address表中，如下所示：**
...
我们可以**根据用户登录名去tb_address表中查询对应的数据**。
...
点击结算页的时候，会**立即创建订单数据**，**创建订单数据会将数据存入到2张表中**，分别是**订单表**和**订单明细表**，**订单明细表是订单中的各个商品信息表**，此处还需要修改商品对应的库存数量。
...
下单的时候，**先添加订单往tb_order表中增加数据，再添加订单明细，往tb_order_item表中增加数据。就是将前台传来的订单数据都封装到tb_order表和tb_order_item表中。有的数据可以从前台页面中封装，有的数据只能通过在serviceImpl方法中封装。**



#### 3.2.1 代码实现

这里先修改changgou-service-order微服务，实现下单操作，这里会**生成订单号，我们首先需要在启动类中创建一个IdWorker对象。**
...
**修改订单微服务添加com.changgou.order.service.impl.OrderServiceImpl,这是一个订单添加方法实现，将前台页面中传过来的Order订单数据进行其他数据填充，根据tb_order表中的详细信息对Order对象进行补充，再从Redis中的购物车数据中取出所有的购物车数据，我们在购物车中肯定有选择框，选择需要进行结算的SKU商品信息，这些选择框也会将相应的SKU商品ID传到后台，我们添加这些商品信息到tb_order_item表中，同时进入结算页面的商品将会在Redis购物车中删除。**

**注意，Order对象中的用户名信息依然要从令牌信息中获取，因此使用TokenDecoder工具类获取该信息。**
...
上面操作只实现了下单操作，但**对应的库存还没跟着一起减少**，我们在**下单之后，应该调用商品微服务，将下单的商品库存减少，销量增加**。每次订单微服务只需要将用户名传到商品微服务，商品微服务通过用户名到Redis中查询对应的购物车数据，然后执行库存减少，**库存减少需要控制当前商品库存>=销售数量**。

![1558305399244](images\1558305399244.png)

如何控制库存数量>=销售数量呢？

当有多个用户一起购买这个商品时，该商品可能会出现**超卖现象**， 即商品的库存不够多个用户购买的商品数，而每个用户都显示已经购买了该商品，超过了库存数量。

**其实可以通过SQL语句实现，MySQL的InnoDB存储引擎中有行级锁的机制，会将该行的数据锁起来，每次只允许一个事务访问该行数据，每次减少数量的时候，加个条件判断库存数量是否大于等于要购买的商品数。**

`where num>=#{num}`即可。



减少库存的操作应该在**商品微服务**中，该工程中一会儿需要查询购物车数据，获取要购买的商品数量，所以需要引入订单的api，在pom.xml中添加如下依赖：
...
**要调用其他微服务，需要将头文件中的令牌数据携带到其他微服务中取**，所以我们**不能使用hystrix的多线程模式**，**要使用信号量模式** ，修改changgou-service-goods的applicatin.yml配置，代码如下：
...
修改changgou-service-goods微服务的`com.changgou.goods.dao.SkuMapper`接口，增加库存递减方法,使用行级锁实现。
...
修改changgou-service-goods微服务的`com.changgou.goods.service.SkuService`接口，传入的参数是由购买的商品的SKUID和数量所组成的map。
...
修改changgou-service-goods的`com.changgou.goods.controller.SkuController`类，**添加库存递减方法，递减的参数有该商品的SKU ID，和该商品的购买数量。这些应该是从Redis购物车中取出来的。**代码看
...
由于创建订单时，订单微服务需要调用这个删减库存的方法，因此在changgou-service-goods-api工程添加`com.changgou.goods.feign.SkuFeign`的实现，代码如下：
...
在创建订单时，调用删减库存方法，修改changgou-service-order微服务的com.changgou.order.service.impl.OrderServiceImpl类的add方法，增加库存递减的调用。
...
3.用户扫码支付认证后，微信支付服务器会通调用前预留的回调地址，并携带支付状态信息。
4.支付系统接到支付状态信息后，将支付状态信息发送给RabbitMQ
5.订单系统监听RabbitMQ中的消息获取支付状态，并根据支付状态修改订单状态
6.如果用户暂时没有支付该订单，那么订单系统就会发送一个延时队列，在30分钟后再查询MQ的订单支付信息。如果那时订单还未支付，则回滚。
