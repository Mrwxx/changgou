...
模板： 将一些重复的内容事先写好，其中某些可能发生变化的内容，采用占位符的方式动态加载（如jsp）

模板引擎技术：可以基于写好的模板，动态给写好的模板加载数据。



Thymeleaf提供了一个用于**整合Spring MVC**的可选模块，在应用开发中，你可以使用Thymeleaf来完全代替JSP或其他模板引擎，如Velocity、**FreeMarker**等， SpringBoot已经淘汰了JSP，如果需要JSP需要自己加入。Thymeleaf的主要目标在于提供一种**可被浏览器正确显示的**、格式良好的模板创建方式，因此也可以用作**静态建模**。你可以使用它**创建经过验证的XML与HTML模板**。相对于编写逻辑或代码，**开发者只需将标签属性添加到模板中即可**。接下来，这些标签属性就会在DOM（文档对象模型）上执行预先制定好的逻辑。
...
所有这些模式都指的是**格式良好的XML文件**，但*Legacy HTML5*模式除外，它允许您处理HTML5文件，其中包含独立（非关闭）标记，没有值的标记属性或不在引号之间写入的标记属性。为了在这种特定模式下处理文件，Thymeleaf将首先执行转换，将您的文件转换为格式良好的XML文件，这些文件仍然是完全有效的HTML5（实际上是创建HTML5代码的推荐方法）[1](https://www.thymeleaf.org/doc/tutorials/2.1/usingthymeleaf.html#fn1)。

另请注意，**验证仅适用于XML和XHTML模板**。

然而，这些并不是Thymeleaf可以处理的唯一模板类型，并且用户始终能够通过**指定在此模式下*解析*模板的方法和*编写*结果的方式来定义他/她自己的模式**。这样，任何**可以建模为DOM树（无论是否为XML）**的东西都可以被Thymeleaf有效地作为模板处理。
...
`<html xmlns:th="http://www.thymeleaf.org">`:这句**声明使用thymeleaf标签**
...
在这里，其实还有一些**默认配置**，比如**视图前缀**：classpath:/templates/,**视图后缀**：.html

默认的thymeleaf配置请到 Project -> External Libraries  -> org.springframework.boot.autoConfigure.thymeleaf.ThymeleafProperties中查看
...
表示**提交的请求地址**为`/test/hello`



(2)th:each

**对象遍历**，功能类似jstl中的`<c:forEach>`标签。 用遍历User对象来测试
...
1. 知道map的key，通过key直接获取value
2. 不知道key,通过循环的方式遍历map
...
搜索的业务流程如上图，用户每次搜索的时候，**先经过搜索业务工程**，**搜索业务工程调用搜索微服务工程**，这里搜索业务工程单独挪出来的原因是它这里**涉及到了模板渲染以及其他综合业务处理，以后很有可能会有移动端的搜索和PC端的搜索**，后端渲染如果直接在搜索微服务中进行，会对微服务造成一定的侵入，不推荐这么做，推荐微服务独立，只提供服务，如果有其他页面渲染操作，可以搭建一个独立的消费工程调用微服务达到目的。
...
由于以后**做搜索都是基于GET请求**，所以我们需要将之前的搜索改成GET请求操作，修改`changgou-service-search`微服务的`com.changgou.search.controller.SkuController`里面的`search`方法，代码如下：
...
通过thymeleaf模板填充，动态修改数据。

首先按照返回的resultMap对象中的rows键值对应的结果集填充数据，该结果集是一个List<SkuInfo>，即每个商品按照SkuInfo中的各个参数填充即可。

th:utext 对应搜素的高亮关键词中的标签不会被当做text显示，而是识别出标签显示高亮。
...
提交搜索条件， action表示表单提交访问的函数地址，作为搜索条件参数提交

搜索 输入框的 name表示搜索条件表单搜索关键词的名称"keywords"，方便在后端函数中取出该关键词进行检索。

value 用于关键词的回显操作，当我们输入一个关键词并点击提交后，该关键词需要回显出来。因此使用存储的搜索条件进行回显，如果没有输入关键词时显示空字符串，这样不会有空异常。
...
通过一个<li> 标签进行遍历搜索结果，然后按照SkuInfo对象中的参数一个个地对应显示上去。
...
分类：`result.searchCategory

品牌：`result.searchBrand`

规格：`result.searchSpec`



下图的逻辑是错误的，应该是若搜索结果集中存在 searchCategory时，代表用户选择了某个分类，这时就不需要显示分类了。品牌，规格同理。
...
用户每次点击搜索的时候，其实在**上次搜索的基础**之上**加上了新的搜索条件**，也就是在上一次请求的URL后面追加了新的搜索条件，我们可以在**后台每次拼接组装出上次搜索的URL**，然后每次**将URL存入到Model中**，页面每次点击不同条件的时候，从Model中取出上次请求的URL，然后再加上新点击的条件参数实现跳转即可。
...
![=](images\1561810836507.png)

![1566882290628](images/1566882290628.png)

th:href 这里是**超链接的语法**，例如：`th:href="@{${url}(price='500-1000')}"`表示请求地址是取`url`参数的值，**同时向后台传递参数price的值为500-100元**。
...
如上图，用户点击条件搜索后，要将选中的条件显示出来，并提供移除条件的`x`按钮,显示条件我们可以从searchMap中获取，**移除其实就是将之前的请求地址中的指定条件删除**即可。
...
对每个x符号指定超链接，链接到移除当前条件的URL中，即删除当前条件的字符串，构建新的URL链接。
...
上图代码是排序代码，需要2个属性，`sortRule`:**排序规则**，ASC或者DESC，`sortField`:**排序的域**，前端每次只需要将这2个域的值**传入到后台即可实现排序**。



(1)后台组装排序URL

每次排序的时候恢复第1页查询，所以url地址我们需要重新拼接,**但是在每次切换排序的时候，不需要之前的排序信息**，因此我们需要记录下没有连接排序参数的URL地址sortUrl，以便直接获取该URL，待到切换排序参数时再向其中添加排序参数。修改SkuController,代码如下：
...
要分页，首先要从前端获取当前所在的页面，以及之前设置搜索条件中每个页面的数据量，然后根据搜索条件构建搜索对象，该对象保存着所有页面总数，当前页面，每个页面的记录数，通过model传到渲染器后，mytheleaf就会显示在分页的地方。
...
注意：每次**如果搜条件发生变化**都要**从第1页查询**，而点击下一页的时候，分页数据在页面给出，不需要在后台拼接的url中给出，所以在拼接url的时候，需要过滤掉分页参数，修改`changgou-web-search`的控制层`com.changgou.search.controller.SkuController`的url拼接方法，代码如下：
...
当**系统审核完成商品**，需要将**商品详情页进行展示**，那么采用静态页面生成的方式生成，并部署到高性能的web服务器中进行访问是比较合适的。所以，开发流程如下图所示：
...
上图是要生成的商品详情页，从图片上可以看出需要查询SPU的3个分类作为面包屑显示，同时还需要查询SKU和SPU信息。整个页面是一个SPU商品的信息，但是在规格和标题处展示的是该SPU商品的每一个SKU商品的规格信息。

所以需要查询分类的Feign接口，查询SKU的Feign接口， 查询SPU的Feign接口。
...
要创建某个SPU商品的详情页，首先要获取该SPU商品信息，对应的所有SKU商品信息，以及商品分类信息，传入model模型中，以供引擎渲染。

还需要使用thymeleaf引擎进行渲染，然后生成该SPU商品对应的html页面，保存到定义好的路径中。
...
注意，由于item.html在template.items目录下，因此要从static目录下获取资源，就要通过../../两级回溯，修改html模板中的所有./
...
Canal监听微服务在监听成功后需要生成静态页面或者删除静态页面，因此需要引入web_item的API。
...
监听类中,**监听商品数据库的tb_spu的数据变化**,当数据变化的时候**生成静态页或者删除静态页**
