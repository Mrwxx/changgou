...
**超卖问题的关键是如果在多用户同时下单时判断当前商品的真正库存数量，当然如果我们使用的是MySQL，可以使用MySQL的行级锁来锁住这个库存数量，但是这里为了性能的提升，我们使用的是Redis，如何在Redis中对商品的库存数量进行控制。**

解决超卖问题，可以利**用Redis队列实现**，**给每件商品创建一个独立的商品个数队列**，例如：A商品有2个，A商品的ID为1001，则可以创建一个队列,key=SeckillGoodsCountList_1001,往该队列中塞2次该商品ID。

**每次给用户下单的时候，先从队列中取数据，如果能取到数据，则表明有库存，可以继续进行下单操作；如果取不到，则表明没有库存，则该用户无法进行下单操作，删除该用户的派对信息，这样就可以防止超卖问题产生了。**



**在解决超卖问题时，也可以使用库存自增的方式，在Redis中初始化一个HashMap，对每个商品ID初始化库存的值，然后在每个用户下单请求该商品时对该商品的库存值自增-1，如果返回的是>=0的数，说明还有库存，可以继续下单，如果返回<0的数，说明没有库存，不能继续下单了。**在我们对Redis进行操作的时候，很多时候，都是先将数据查询出来，在内存中修改，然后存入到Redis，在并发场景，会出现数据错乱问题，为了控制数量准确，我们单独将商品数量整一个自增键，自增键是线程安全的，所以不担心并发场景的问题。
...
修改SeckillGoodsPushTask,添加一个pushIds方法，**用于将指定商品ID放入到指定的数组中，然后将该数组作为商品的库存数组放入商品的库存队列中，等待用户下单时从库存队列中获取库存，如果能够获取到库存说明还有库存，则用户进行下单操作，否则没有库存，删除用户的排队信息，以及删除用户的排队状态。**，代码如下：
...
修改多线程下单方法，分别修改数量控制，以及售罄后用户抢单排队信息的清理，**如果能够获取到库存说明还有库存，则用户进行下单操作，否则没有库存，删除用户的排队信息，以及删除用户的排队状态。**



**针对并发请求下单并删减库存时，可能会出现多个请求同时在内存中并发删减库存，导致库存的状态不一致的问题**，即如果两个请求获取到商品库存2，进入下单状态，并同时进行删减库存操作，这样每个请求下单后的库存状态都是2 -> 1 ，而不是2->1->0的顺序，那么就会产生数据不一致问题。**我们直接采用之前Redis中的库存队列，判断该库存队列的长度即可，因为Redis是单线程的，所以通过该队列来获取库存数据不会有数据不一致问题。然后在同步库存数据的时候，按照队列中的库存来更新。**
...
<img src="images\1557113836465.png" alt="1557113836465" style="zoom:200%;" />

**完成秒杀下订单后，进入支付页面，此时前端会每3秒中向后台发送一次请求用于判断当前用户订单是否完成支付**，如果**完成了支付，则需要清理掉排队信息，并且需要修改订单状态信息**。
...
下单成功后，会**跳转到支付选择页面**，在**支付选择页面要显示订单编号和订单金额，所以我们需要在下单的时候，将订单金额以及订单编号信息存储到用户查询对象中**。
...
支付回调这一块代码已经实现了，但之前实现的是订单信息的回调数据发送给MQ，指定了对应的队列，不过**现在需要实现的是秒杀信息发送给指定队列，所以之前的代码那块需要动态指定队列**。
...
在**微信支付统一下单API中，有一个附加参数**,如下：

```properties
attach:附加数据,String(127)，在查询API和支付通知中原样返回，可作为自定义参数使用。
```

我们可以**在创建二维码的时候，指定该参数，该参数用于指定回调支付信息的对应队列**，**每次回调的时候，会获取该参数，然后将回调信息发送到该参数对应的队列去**。
...
修改SeckillOrderServiceImpl，创建一个关闭订单实现方法，**首先要在Redis中删除这个排队订单，然后获取这个订单所对应的商品信息，在一般情况下库存不为0的商品都还有库存能够获取到该商品的信息，除非是当前商品库存为0，那么在Redis中已经没有对应的商品信息了，只能从MySQL中获取该商品信息，然后设置库存为1，对于能够从Redis中获取的商品信息，设置库存+1即可，当然别忘了我们之前设置的库存队列，向该队列中添加一个库存数字。最后还需要删除这个订单所对应的订单状态。**
...
系统中的业务失败之后，需要重试
这些场景都非常常见，我们可以思考，比如第二个需求，系统创建了预约之后，需要在预约时间到达前一小时提醒被预约的双方参会。那么一天之中肯定是会有很多个预约的，时间也是不一定的，假设现在有1点 2点 3点 三个预约，如何让系统知道在当前时间等于0点 1点 2点给用户发送信息呢，**是不是需要一个轮询，一直去查看所有的预约，比对当前的系统时间和预约提前一小时的时间是否相等呢？这样做非常浪费资源而且轮询的时间间隔不好控制**。如果我们**使用延时消息队列呢，我们在创建时把需要通知的预约放入消息中间件中，并且设置该消息的过期时间，等过期时间到达时再取出消费即可。**

Rabbitmq实现延时队列一般而言有**两种形式：**
...
```properties
1.用户抢单，经过秒杀系统实现抢单，下单后会将向MQ发送一个延时队列消息，包含抢单信息，延时半小时后才能监听到
2.秒杀系统同时启用延时消息监听，一旦监听到订单抢单信息，判断Redis缓存中是否还存在该订单信息，如果存在，说明该订单依然没有支付，则进行库存回滚操作。
3.秒杀系统还启动支付回调信息监听，如果支付完成，则将订单同步到MySQL，如果没完成，清理排队信息回滚库存
4.每次秒杀下单后调用支付系统，创建二维码，如果用户支付成功了，微信系统会将支付信息发送给支付系统指定的回调地址，支付系统收到信息后，将信息发送给MQ，第3个步骤就可以监听到消息了。
```
