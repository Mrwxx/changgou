...
重启后发现重启启动失败了，这时什么原因呢？这与我们刚才修改的配置有关，因为elasticsearch在**启动的时候会进行一些检查**，比如最多打开的文件的个数以及虚拟内存区域数量等等，如果你放开了此配置，意味着需要打开更多的文件以及虚拟内存，所以我们还需要系统调优 
...
修改vi /etc/sysctl.conf，追加内容 (限制一个进程可以拥有的**VMA(虚拟内存区域)**的数量 )
...
将ik目录拷贝到docker容器的plugins目录下**（Docker 之间文件的传递）**
...
IKAnalyzer.cfg.xml  修改配置文件，添加自定义分词文件



自定义停用词汇：

同样的在 IKAnalyzer.cfg.xml  中修改配置文件，添加停用词汇文件
...
要使用Kibana，您必须至少**配置一个索引**。索引用于标识Elasticsearch索引以运行搜索和分析。它们还用于配置字段。 

![1554423078755](images/1554423078755.png)

我们修改索引名称的匹配方式即可，下面2个选项不用勾选。点击create，会展示出当前**配置的索引的域信息**，如下图：
...
**Query DSL是一个Java开源框架用于构建类型安全的SQL查询语句**。采用**API代替传统的拼接字符串**来构造查询语句。目前Querydsl支持的平台包括JPA,JDO，SQL，Java Collections，RDF，Lucene，Hibernate Search。elasticsearch提供了一整套基于JSON的查询DSL语言来定义查询。
...
1. 首先在搜索API中创建一个POJO（SkuInfo），对应的是查询出来的SKU，但是要添加与 ES的映射注解
2. 创建Feign，实现查询所有的SKU
3. 在搜索微服务中调用Feign，查询所有的SKU，并将SKU转换为SkuInfo集合
4. 调用DAO层（继承 ElastricSearchRepositiry接口)，并将所有SkuInfo集合存入ES索引库中
...
FieldType.keyword表示这是关键词，不需要分词，直接搜索即可。有些属性没有添加@Field注解，会自动作为一个域添加到当前索引的当前类型之中。
...
修改changgou-service-search工程，创建com.changgou.search.service.impl.SkuServiceImpl,实现Sku数据导入到ES中，由于最终搜索出来的规格参数的键值是根据不同商品的种类来确定的，因此规格参数的键值是动态变化的。我们需要根据搜索出来的SKU的商品种类来确定不同的商品规格参数，然后将SKU的spec字符串解析为map，该字符串本身就是Json格式的，因此直接使用jackson来解析为map即可，将每个SKU商品的spec字符串解析为一个map，存储到该SKU对应的SkuInfo对象的specMap属性中，这样该SKU的每个规格参数就保存到ES的索引库中了。
...
1. goods微服务中添加通过status查询所有SKU的接口findByStatus
2. 将该接口添加到goods_api工程中的feign包中，暴露出这个接口
3. search微服务需要调用goods微服务的findByStatus接口，因此直接通过feign调用goods_api中的feign接口即可
4. 获取SKU后需要转换为SKUINFO，对应ES索引库
5. 将SKUINFO存储到ES索引库中
...
我们先使用SpringDataElasticsearch实现一个简单的搜索功能，先实现根据关键字搜索，从上面搜索图片可以看得到，每次搜索的时候，除了关键字外，还有可能有品牌、分类、规格等，后台接收搜索条件使用Map接收比较合适。后台返回的结果也设置为map，返回商品的各种信息。



搜索流程：

1. 输入关键词，同时还有各种搜索条件，这些条件需要封装为一个条件搜索对象  NativeSearchQueryBuilder
2. 用ElasticSearchTemplate的查询方法进行搜索，传入条件搜索对象，以及会生成的响应结果类型，最后生成的是一个分页对象AggregatedPage，通过这个分页对象来获取各种商品SKU信息。
3. 最后可以封装一个map来封装所有的返回信息
...
使用get而不是用post，是因为可能会从浏览器直接输入keywords进行搜索，因此无需使用post，同时还有可能没有输入关键词，因此要设置为false。
...
看下面的SQL语句，我们在执行搜索的时候，第1条SQL语句是执行搜，第2条语句是**根据分类名字分组查看有多少分类**，大概执行了2个步骤就可以获取数据结果以及分类统计，我们可以发现他们的搜索条件完全一样。
...
将共性的代码块抽取出来封装成私有函数，只供该实现类使用，选中代码块，按住 CTRL + ALT + M ，封装为函数

具体的代码请到search.service.SkuServiceImpl中查看，以下代码并非最终结果。
